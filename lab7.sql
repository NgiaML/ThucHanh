...
BEGIN
    ...
    BEGIN
        PRINT 'Ten hang san xuat da ton tai. Vui long kiem tra lai!'
    END
    ELSE
    BEGIN
...
EXEC sp_InsertHangsx 'HSX01', 'Samsung', 'Hàn Qu?c', '0123456789', 'contact@samsung.com'
...
BEGIN
    ...
    END
END
EXEC sp_ThemSuaSanPham 'SP01', 'H01', 'Galaxy Note 11', '50', '??','1900000.00','Chiec','Hàng c?n c?o c?p'
...
BEGIN
    ...
    BEGIN
        PRINT 'Hang khong ton tai trong bang'
        RETURN
    END
...
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM Sanpham WHERE masp = @masp)
    BEGIN
        PRINT 'Ma san pham khong ton tai trong bang Sanpham.'
        RETURN
    END
    ...
    IF NOT EXISTS (SELECT * FROM Nhanvien WHERE manv = @manv)
    BEGIN
        PRINT 'Ma nhan vien khong ton tai trong bang Nhanvien.'
        RETURN
    END
...
BEGIN
    ...
    IF NOT EXISTS(SELECT * FROM Nhanvien WHERE manv = @manv)
    BEGIN
        PRINT 'Khong tim thay nhan vien voi ma ' + CAST(@manv AS NVARCHAR)
        RETURN
    END
    ...
    PRINT 'Da xoa nhan vien voi ma ' + CAST(@manv AS NVARCHAR)
END
...
BEGIN
  ...
  BEGIN
    PRINT 'Khong tim thay san pham da xoa!'
    RETURN;
  END
