-- cau 1
CREATE FUNCTION tenhangsx(@masp nvarchar(20))
	RETURNS nvarchar(20)
AS
BEGIN 
	DECLARE @ten nvarchar(20)
	SET @ten = (select hsx.tenhang from Hangsx hsx
		inner join Sanpham sp 
		ON hsx.mahangsx = sp.mahangsx
		WHERE sp.masp = @masp)
	RETURN @ten
END  

SELECT dbo.tenhangsx('SP02')
...
Câu 4
CREATE FUNCTION TongGTNhap(@x nvarchar(20), @y nvarchar(20))
	RETURNS INT
AS
BEGIN 
	DECLARE @sum int
	SET @sum = (select SUM(dongiaN) from Nhap
				WHERE ngaynhap >= @x
				AND ngaynhap <= @y)
	RETURN @sum
END  

SELECT dbo.TongGTNhap('2020-05-15', '2020-07-07')
-- cau 5
